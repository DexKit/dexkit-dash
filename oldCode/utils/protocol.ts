...
import { EXCHANGE, NETWORK, EXCHANGE_NAME_ON_URL } from "shared/constants/AppEnums";
import { DEXTOOLS_API_URL, UNISWAP_INFO_API_URL, SUSHISWAP_INFO_API_URL } from "shared/constants/AppConst";
...
export const GET_PROTOCOL_PAIR_URL = (network: NETWORK, exchange: EXCHANGE, data: dataProtocolPair) => {
    switch (exchange) {
      case EXCHANGE.UNISWAP:
        return `/${network}/protocol-explorer/${EXCHANGE.UNISWAP}/pair-explorer/${data?.contract || data?.address}`;
      case EXCHANGE.ZEROX:
...
export const GET_PROTOCOL_TOKEN_URL = (network: NETWORK, token: Token, exchange: EXCHANGE) => {
  switch (exchange) {
    case EXCHANGE.UNISWAP:
...
export const GET_AMM_ANALYTICS = ( exchange: EXCHANGE, address: string,) => {
  switch (exchange) {
    case EXCHANGE.UNISWAP:
      return {url: `${UNISWAP_INFO_API_URL}/pair/${address}`, icon: '/images/uniswap.png' } ;
   case EXCHANGE.SUSHISWAP:
        return {url: `${SUSHISWAP_INFO_API_URL}/pair/${address}`, icon:  '/images/sushiswap.png'};
    default:
      return null;
  }
}
