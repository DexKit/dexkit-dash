...
import { NotificationAction, NotificationActions } from "types/actions/Notification.actions";
  ...
  switch(action.type){
    ...
    case NotificationAction.REMOVE_NOTIFICATION: {
      ...
      if(notification != null){
        ...
        if(selected != null){
          selected = selected?.id?.toString() === notification?.id?.toString() ? undefined : selected;
        }
        return {
          notifications: getNotificationList(),
          selected: selected
    ...
    case NotificationAction.CHECK_NOTIFICATION:{
      ...
      if(notification != null){
        let selected: Notification | undefined = state?.selected;
        if(selected != null){
          selected = selected?.id?.toString() === notification?.id?.toString() ? notification : selected;
        }
        return {
          notifications: getNotificationList(),
          selected: selected
    ...
    case NotificationAction.CHECK_ALL_NOTIFICATION:{
      ...
      let selected: Notification | undefined = state?.selected;
      if( selected != null){
        selected = notifications.find( x => x.id?.toString() === selected?.id?.toString());
      }
      return {
        notifications,
        selected
