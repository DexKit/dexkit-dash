...
const TransactionList: React.FC<Props> = (props) => { 
  ...
  const {loading, error, data, currentPage, rowsPerPage, rowsPerPageOptions, onChangePage, onChangePerPage} = useTransactionList({address});
            ...
            <Card>
              ...
              { loading ? <LoadingView /> : error ? <ErrorView message={error.message} /> : (
                <TransactionTable 
                  ...
                  onChangePerPage={(perPage)=> onChangePerPage(perPage)} 
              />
              )}
            </Card>
          </Box>
