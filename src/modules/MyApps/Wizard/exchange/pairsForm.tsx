...
const PairComponent: React.FC<PairComponentProps> = (props) => {
  ...
  const maxAmountError = useCallback((): string | undefined => {
    if (positiveValidator(config?.minAmount)) {
      return (config?.minAmount ?? 0) >= (config?.maxAmount ?? 0)
  ...
  const _onBlur = (
    ...
    key: string | keyof ConfigPairMetaData,
    errors: error,
    hasProperty: boolean,
    config: ConfigPairMetaData,
    setConfig: React.Dispatch<
      React.SetStateAction<ConfigPairMetaData | undefined>
    >,
  ...
  useEffect(() => {
    ...
    if (errors != null) {
      const _valid =
        Object.values(errors).reduce((pre, cur) => pre && cur == null, true) &&
        Object.values(errors?.config ?? {}).reduce(
  ...
  const otherFields = Object.keys(config ?? {}).filter((f) =>
    exclude.every((x) => x !== f),
  );
  return (
    <>
      ...
      <Grid item xs={12} md={6} sm={6}>
        <TextField
          ...
          InputProps={{
            endAdornment: (
              <InfoComponent text={getHelpText(HELP_TEXT_PAIR, 'base', 0)} />
            ),
          }}
          disabled
        />
      ...
      <Grid item xs={12} md={6} sm={6}>
        <TextField
          ...
          InputProps={{
            endAdornment: (
              <InfoComponent text={getHelpText(HELP_TEXT_PAIR, 'quote', 0)} />
            ),
          }}
          disabled
        />
      ...
      {config
        ? otherFields?.map((key) => {
            ...
            const hasProperty =
              !isObjectNullOrEmpty(errors) &&
              checkProperty<ConfigError>(errors?.config, _key);
            return (
              <Grid item xs={12} md={6} sm={6} key={`pair(${index}).${key}`}>
                <PairItemComponent
                  ...
                  inputProps={{
                    ...
                    max:
                      _key === 'minAmount'
                        ? config?.maxAmount ?? MAX_NUMBER
                        : MAX_NUMBER,
                    step:
                      _key === 'maxAmount' || _key === 'minAmount'
                        ? Math.pow(
                            10,
                            -Math.max(
                              Math.min(config?.pricePrecision ?? 4, 8),
                              4,
                            ),
                          )
                        : 1,
...
const PairsForm: React.FC<Props> = (props) => {
  ...
  const AccordionLabelComponente = memo((props: AccordionLabel) => {
    ...
    const text =
      base && quote && `${base.toUpperCase()}/${quote.toUpperCase()}`;
    return (
      <Typography
  ...
  return (
    <GridContainer>
      {pairs != null
        ? pairs.map((pair: CurrencyPairMetaData, i: number) => (
              ...
              <Accordion>
                <AccordionSummary
                  ...
                  <AccordionActions>
                    <CustomIconButton
                      aria-label={`add(${i})`}
                      onClick={($e) => addPair(i, $e)}
                      disabled={Boolean(editable) ? !startValidation : true}>
