...
export const ItemComponent: React.FC<ItemComponentProps> = ({
  ...
  const isRequired =
    getFieldProperties(fieldName)?.isRequired !== undefined
      ? getFieldProperties(fieldName)?.isRequired
      : true;
        ...
        <Grid item xs={12} md={6} sm={6}>
          <TextField
            ...
            onBlur={() => {
              if (
                error != null &&
                (error[fieldName] ?? '').toString().length < 5
              ) {
                setError({
                  [fieldName]: `Color in ${capitalize(
                    fieldName,
                    '_',
                  )}  is invalid!`,
                });
