...
interface ArtistCollectionsSearchProps {
  ...
  onSave: ( $e: React.MouseEvent<HTMLButtonElement, MouseEvent>| undefined,
    ownerAddress?: string) => void;
}
...
const columns = [
  ...
  {
    field: 'slug', headerName: 'Slug URL', width: 150 
  },
  {
...
export function ArtistCollectionsSearch(props: Props) {
  const { artistAddress, validator, update, onSave, collections: startCollections } = props;
  ...
  const [collections, setCollections] = useState<CollectionInfo[]>(startCollections ?? []);
  const [selecteds, setSelecteds] = useState<CollectionInfo[]>( startCollections ?? []);
  ...
  useEffect(() => {
    const _rows = collections?.map( (collection,i) => {
      return {
        id: i+1,
        address: collection?.primary_asset_contracts?.length > 0 ? 
        collection?.primary_asset_contracts[0]?.address : undefined,
    ...
    <>
      <Grid item xs={12} key="search-collections">
        <Box display="flex" width="100%" marginY="1.5rem">
          <TextField
            ...
            variant="outlined"
            InputProps={{ endAdornment: (<InfoComponent text={getHelpText(HELP_TEXT_ARTIST_COLLECTIONS, 'address', 0)} />) }}
            disabled={loading}
          />
          <IconButton 
          aria-label="send" 
      ...
      <Grid item xs={12} key="collections-table">
        <Box height={300} width="100%" marginY="1.5rem">
          ...
          loading={loading}
          onCellClick={($e) => {
            if($e?.field === 'imageUrl' && $e?.getValue($e?.field)?.toString()){
              window.open($e?.getValue($e?.field)?.toString());
            } else if($e?.field !=='__check__'  && $e?.field !== 'id' && $e?.getValue('slug')?.toString()){
              window.open(`https://opensea.io/collection/${$e?.getValue('slug')?.toString()}`);
            }
          }}
          onSelectionModelChange={($e) => {
            setSelecteds($e?.selectionModel?.map(
              id => {
      ...
      <Grid item xs={12} md={12}>
        { searchfailed && 
          <MessageView 
          variant='warning' 
          message={searchfailed} anchorOrigin={{vertical: 'bottom', horizontal: 'right' }}/>}
      </Grid>
    </>
  )
