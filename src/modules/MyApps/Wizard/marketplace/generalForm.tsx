...
const GeneralForm: React.FC<Props> = (props) => {
  ...
  useEffect(() => {
    ...
    _errors = keys.reduce((obj, k) => {
      return {
        ...obj,
        [k as keyof error]: validatorToOtherFields(
          k as keyof GeneralConfig,
          _errors ?? {},
        )[k],
      } as error;
    }, _errors);
    const _valid = Object.values(_errors).reduce((prev, curr) => {
        ...
        <Grid
          ...
          <TextField
            ...
            InputProps={{
              endAdornment: (
                <InfoComponent
                  text={getHelpText(HELP_TEXT_SOCIAL_NETWORK_LINKS, key, 0)}
                />
              ),
            }}
          />
