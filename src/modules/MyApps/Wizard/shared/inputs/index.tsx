...
interface WizardTextInputProps {
  ...
  value?: string;
  placeholder?: string;
  onChange?: ( event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement> | string | undefined, field: string) => void;
...
export const WizardTextInput: FC<WizardTextInputProps> = (props) => {
  ...
  return (
    <CustomTextInput
      ...
      fullWidth
      label={required != null? <CustomLabel text={label} required={true} /> : label}
      variant='outlined'
      value={value}
      placeholder={placeholder}
      helperText={!valided ? (error != null ? error[property]: undefined) : undefined}
      minLength={minLength}
      maxLength={maxLength}
      ...
      validator={(value) => error == null || (error != null &&  error[property] == null)}
      onChange={($e) => {
        if (Boolean(disabled)) {
          return;
        }
