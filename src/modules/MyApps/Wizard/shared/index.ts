...
export function getHelpText<T>(helpText: HelpText<T>, key: string, index: number = 0): string{
  if( helpText == null || index < 0 || key == null || key.length === 0)
  {
    return '';
  }
  type K = keyof typeof helpText;
  if(!((key as K) in helpText) || helpText[(key as K)] == null || index >= helpText[(key as K)].length){
    return '';
  }
