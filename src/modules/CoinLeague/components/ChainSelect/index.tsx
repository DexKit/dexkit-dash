...
export const ChainSelect = ({fullWidth}: {fullWidth?: boolean}) => {
  const {chainId} = useLeaguesChainInfo();
  const history = useHistory();
  const updateLeaguesChain = useCallback(
    (e) => {
  ...
  return (
    <Tooltip
      title={
        <IntlMessages
          id='app.coinLeagues.chainSelected'
          values={{chainName: `${GET_CHAIN_ID_NAME(chainId)}`}}
        />
      }>
