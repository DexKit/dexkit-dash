...
import {
  ...
  GET_EXCHANGE_FROM_URL,
  GET_URL_NAME_EXCHANGE,
  truncateAddress,
} from 'utils';
import {
  EXCHANGE,
...
const TokenExplorer: React.FC<TokenProps> = (props) => {
  ...
  const [exchange, setExchange] = useState(
    searchParams.get('protocol')
      ? GET_EXCHANGE_FROM_URL(searchParams.get('protocol') as any)
      : defaultExchange,
  );
  ...
  const onClickSearch = (token: Token) => {
    ...
    const defaultExchange =
      net === EthereumNetwork.ethereum ? EXCHANGE.UNISWAP : EXCHANGE.PANCAKEV2;
    getPairContractFromExchange(
      ...
      if (pairContract) {
        ...
        history.push({
          pathname: `/protocol-explorer/pool-explorer/${pairContract}`,
  ...
  const onClick = (p: EXCHANGE) => {
    getPairContractFromExchange(
      baseAddress,
      getNativeCoinWrappedAddressFromNetworkName(networkName),
      networkName,
      p,
    ).then((pairContract) => {
      if (pairContract) {
        ...
        <GridContainer>
          <Grid item xs={12} md={6}>
            <PageTitle
              ...
              subtitle={{
                name: !isMobile ? truncateAddress(address) : '',
                hasCopy: address,
              }}
              network={networkName}
              shareButton={true}
          ...
          <Grid item xs={12} md={6} alignItems={'flex-end'}>
            <ProtocolSwitcher
              networkName={
                networkName as EthereumNetwork.bsc | EthereumNetwork.ethereum
              }
              protocol={exchange}
              onClick={onClick}
            />
              ...
              <Grid item xs={12} md={12}>
                ...
                <TabPanel value='top-pools'>
                  {data ? (
                    ...
                    <Typography component='h1' color={'primary'}>
                      No data available for this token on{' '}
                      {GET_EXCHANGE_NAME(exchange)}
                    </Typography>
                  )}
                </TabPanel>
