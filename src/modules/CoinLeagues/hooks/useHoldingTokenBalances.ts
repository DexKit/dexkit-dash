...
import { BITTOKEN_MULTIPLIER_HOLDING, DEXKIT_MULTIPLIER_HOLDING } from '../constants';

type TokenWithMultiplier = Token & {
  multiplierHolding: BigNumber;
}

export const useHoldingTokenBalances = (
  account?: string,
) => {
  ...
  const tokenBalancesQuery = useQuery(
    ...
    async () => {
      ...
      const [, tb] = await getTokenBalances(tokens.map(t=> t.address), account, networkProvider);
      return tokens.map((t) => {
        return {
          token: t,
