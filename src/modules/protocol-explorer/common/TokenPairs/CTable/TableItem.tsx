...
const TableItem: React.FC<TableItemProps> = ({ row, exchange, networkName }) => {
  ...
  return (
    <TableRow hover role='checkbox' tabIndex={-1} key={row.address}>
      ...
      <TableCell align='left' className={classes.tableCell}> {row.quoteVolume24.toFixed(3)} <Link href={GET_PROTOCOL_TOKEN_URL(networkName, row.quoteToken, exchange)}> {row.quoteToken.symbol} </Link></TableCell>
