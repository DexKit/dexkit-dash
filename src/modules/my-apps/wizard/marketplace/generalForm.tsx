...
const GeneralForm: React.FC<Props> = (props) => {
  ...
  const validatorToOtherFields = useCallback((key: keyof GeneralConfig) => {
    const { title, icon, domain, feeRecipient, feePercentage } = fields;
    if (errors != null) {
      switch(key){
        case 'title': {
          errors.title = (title ?? "").length > 3 ? undefined : "Title invalid!";
          break;
        }
        case 'icon': {
          errors.icon = urlValidator(icon ?? "") ? undefined : "Icon URL is invalid!";
          break;
        }
        case 'domain': {
          errors.domain = urlValidator(domain ?? "") ? undefined : "Domain URL is invalid!";
          break;
        }
        case 'feePercentage': {
          const perc = Number(feePercentage);
          errors.feePercentage = feePercentage != null && (perc < 0 ||  perc > 100  || isNaN(perc) || !isFinite(perc)) ? 
            "Fee Percentage number is invalid" : undefined;
          break;
        }
        case 'feeRecipient': {
          errors.feeRecipient = isAddress(feeRecipient ?? "") ? undefined : "Invalid Fee Address";
          break;
        }
      }
      ...
      .reduce((pre, cur): boolean => {
        ...
        const fieldValid = fields[(cur as k)] != null && (fields[(cur as k)]?.toString()?.length ?? 0) > 0;
        // console.log(cur, fieldValid);
  ...
  const socialvalidator = useCallback((key: keyof SocialNetworks) => {
    ...
    if(social != null && errors){
      ...
      const errorMsg = socialUrl != null && socialUrl.length > 0 ? 
        ( !urlValidator(socialUrl) ? `${capitalize(key, '_', ' ')} is invalid!` : undefined ) : 
        errors[(key as k)];
