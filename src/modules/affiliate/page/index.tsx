      ...
      <Box pt={{xl: 4}}>
        ...
        <GridContainer>
          <Grid  item xs={12} md={8} xl={8}>
            ...
            <Typography variant='h4'>Configure Affiliate <SettingsIcon color={'primary'} /> </Typography>
                <Box mb={2} display={'flex'} justifyContent={'flex-end'} alignItems={'center'}>
                {kitValue >= 200 &&  <Tooltip title="Account eligible to receive rewards. Congrats!">
                      <CheckIcon style={{ color: green[500], marginRight: '2px' }} />
                    </Tooltip>}
                    {kitValue < 200 &&  <Tooltip title="Account not eligible to receive rewards, BUY KIT till eligible">
                      <CloseIcon style={{ color: red[500], marginRight: '2px' }} />
                    </Tooltip>}
                    <Typography variant='h6'>{kitValue} of 200 KIT</Typography>
                </Box>
              
                <Typography ><Tooltip title="Account that will receive rewards"><span style={{fontWeight: "bold"}}>Account Receive Rewards:</span></Tooltip> {account}</Typography>
               
                <Typography >  <span style={{fontWeight: "bold", marginRight: '10px'}}>Chain:</span> 
                    <Select
                      labelId="chainSelect"
                      id="chainSelectId"
                      value={chain}
                ...
                <Typography ><span style={{fontWeight: "bold"}}>Affiliate Link:</span> <Link href={affiliateLink}  target={'_blank'}>{affiliateLink} <OpenInNewIcon/></Link> <ButtonCopy copyText={affiliateLink}  titleText='Copied to clipbord !'></ButtonCopy></Typography>
            </AppCard>
          </Grid>
