...
const CoinService: CoinInterface = {
    ...
    getBalance: async (address: string) => {
      ...
      const balanceBTC = (new BigNumber(balanceSat)).dividedBy(10 ** CoinService.precision);
      return balanceBTC;
    },
    ...
    getTransactions: (address: string) => {
        return CoinService.fetchUnspent(address);
    ...
    getPrivateKeyForAddress: (address: string) => {
        const pk =  privateKeys.find(p => p.address.toLowerCase() === address?.toLowerCase());
        if(pk){
            return pk.key;
        }else{
            return false;
    ...
    setPrivateKey: async (encryptedSeed: string, passphrase: string, index: number = 0) => {
        const seed = (await browserpassworder.decrypt(passphrase, encryptedSeed)).seed;
        const root = bip32.fromSeed(new Buffer(seed), bitcoin_network);
        const {publicKey, privateKey} = root.derivePath(CoinService.getPath()).derive(0).derive(index);
        const wallet = bitcoin.payments.p2pkh({pubkey: publicKey, network: bitcoin_network});
        const pk =  privateKeys.find(p => p.address.toLowerCase() === wallet?.address?.toLowerCase());
        if(pk){
            return false;
        }else{
            privateKeys.push({key: privateKey?.toString() as string, address: wallet?.address as string, index })
