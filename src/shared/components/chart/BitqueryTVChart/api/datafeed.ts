...
let cacheCoin = new Map<string, string>();
...
export default {
   ...
   async getBars (symbolInfo: any, resolution: any, from: any, to: any, onHistoryCallback: any, onErrorCallback: any, first: any){
        try{
            if (resolution==='1D' || resolution === 'D') {
                resolution = 1440;
            }
            const network = symbolInfo.ticker.split(':')[0] as EthereumNetwork.bsc | EthereumNetwork.ethereum;
            ...
            const groups = response2.data.data.ethereum.dexTrades.reduce((group: any, el: any) => {
                const date = Math.floor(new Date(el.timeInterval.minute).getTime() / 1000);
                if (!group[date]) {
                    group[date] = [];
                }
                group[date].push(el);
            ...
            const bars = Object.keys(groups).filter(date => groups[date].length === 2).map((date) => {
                ...
                let priceUSD;
                // We find the principal pair and multiply by quotePrice in USD
                if (data[0].baseCurrency.address.toLowerCase() === base.toLowerCase() && data[0].quoteCurrency.address.toLowerCase() === wrappedNative[network].toLowerCase()) {
