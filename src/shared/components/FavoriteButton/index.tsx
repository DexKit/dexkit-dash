...
import { getTokenById, getTokenCoingeckoItemList, getTokens } from "services/rest/coingecko";
...
export const FavoriteButton =  ({token, detailToken}: Props) => {
    ...
    useEffect(()=>{
        const address = token?.address;
         if(address && !detailToken ){
            getTokenCoingeckoItemList(address)
                .then(d => {
                    if(d){
                        setTk(d)
                    }
    ...
    return  (
        (token && tk) ? <Tooltip title="Add to Favorites">
