...
import {
  GetBalanceBlock,
  GetBalanceBlockVariables,
} from 'services/graphql/bitquery/balance/__generated__/GetBalanceBlock';
import {
  GetMySingleBalanceHistory,
  GetMySingleBalanceHistoryVariables,
} from 'services/graphql/bitquery/balance/__generated__/GetMySingleBalanceHistory';
...
export const useBalanceChart = (balances: MyBalances[]) => {
  ...
  useEffect(() => {
    if (!selectToken && balances.length > 0) {
      ...
      setSelectToken(
        balances[0].currency?.address ?? balances[0].currency?.symbol,
      );
    }
  }, [balances]);
  ...
  const handleSelectToken = (token: string, net: EthereumNetwork) => {
    if (allData && allData[token] && allData[token].length) {
      const dataFn: ChartTick[] =
        allData[token].map((e: any) => {
          return {
  ...
  useEffect(() => {
    ...
    client
      .query<GetBalanceBlock, GetBalanceBlockVariables>({
  ...
  useEffect(() => {
    if (block && account && network && selectToken && balances.length > 0) {
      client
        .query<GetMySingleBalanceHistory, GetMySingleBalanceHistoryVariables>({
        ...
        .then((b) => {
          const data = b.data.ethereum?.address[0].balances?.reduce<{
            [idx: string]: ChartTick[];
          }>((acc, current): any => {
            const idx = isNativeCoinFromNetworkName(
              current.currency?.symbol.toUpperCase() ?? '',
              network,
            )
              ? current.currency?.symbol.toUpperCase()
              : current.currency?.address;
            const v: ChartTick[] = [];
