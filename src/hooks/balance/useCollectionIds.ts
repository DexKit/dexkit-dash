...
import {
  GetNFTIDs,
  GetNFTIDsVariables,
} from 'services/graphql/bitquery/nft/__generated__/GetNFTIDs';
...
export const useCollectionIds = (
  ...
  const tokenIdsQuery = useQuery(
    ['GetCollectionTokenIds', address, account, networkName],
    () => {
      if (address && account) {
        return client
          .query<GetNFTIDs, GetNFTIDsVariables>({
            query: GET_NFT_IDs,
            variables: {
              receiver: account,
              network: networkName,
          ...
          .then((d) => {
            if (
              d &&
              d.data.ethereum &&
              d.data.ethereum?.transfers &&
              d.data.ethereum?.transfers
            ) {
              return d.data.ethereum?.transfers
                .filter((t) => t.entityId)
