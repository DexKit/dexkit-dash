...
export const useSingleBalance = (address: string, network: EthereumNetwork, defaultAccount?: string) => {
  ...
  const [data, setData] = useState<GetSingleBalance_ethereum_address_balances>( );

  const {loading, error, data: dataFn} = useQuery<GetSingleBalance, GetSingleBalanceVariables>(BITQUERY_SINGLE_BALANCE_INFO, {
    variables: {
      network,
  ...
  useEffect(()=> {
    if(dataFn && dataFn.ethereum?.address 
