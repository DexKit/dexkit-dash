...
export const useSufficientBalance = (
  amountOrUsd: number,
  currency?: GetMyBalance_ethereum_address_balances_currency,
) => {
  const {loading, error, data: balance} = useBalance();
  const _amount =
    amountOrUsd != null && isFinite(amountOrUsd) && !isNaN(amountOrUsd)
      ? amountOrUsd
      : 0;
  if (currency != null) {
    const currencyBalance =
      balance?.filter(
        (b) =>
          b?.currency?.symbol?.toUpperCase() === currency.symbol.toUpperCase(),
      ) ?? [];
