...
import { GetTransferList, GetTransferListVariables, GetTransferList_ethereum_transfers } from "services/graphql/bitquery/history/__generated__/GetTransferList";
...
export const useTransferList = ({address, networkName}: Props) =>{
  ...
  const { loading, error, data: dataFn } = useQuery<GetTransferList, GetTransferListVariables>(BITQUERY_TRANSFER_LIST, {
    variables: {
      network: networkName,
      address: address,
  ...
  useEffect(() => {
    if (dataFn && dataFn?.ethereum?.transfers && dataFn?.ethereum?.transfers.length) {


      const transfers: Transfers[] = dataFn.ethereum.transfers.map((e: GetTransferList_ethereum_transfers) => {
