...
import { GetAllTradeHistoryList, GetAllTradeHistoryListVariables, GetAllTradeHistoryList_ethereum_dexTrades } from "services/graphql/bitquery/history/__generated__/GetAllTradeHistoryList";
...
export const useAllTradeHistory = ({address,  networkName}: Props) =>{
  ...
  const [data, setData] = useState<GetAllTradeHistoryList_ethereum_dexTrades[]>();
  const [totalRows, setTotalRows] = useState<number>();
  // If there is no baseCurrency the API returns duplicated values, so we just multiply and then filter

  const { loading, error, data: dataFn } = useQuery<GetAllTradeHistoryList, GetAllTradeHistoryListVariables>(BITQUERY_ALL_TRADE_HISTORY_LIST, {
    variables: {
      network: networkName,
