...
import {
  GetOrderInfo,
  GetOrderInfoVariables,
} from 'services/graphql/bitquery/history/__generated__/GetOrderInfo';
...
export const useOrderInfo = ({hash}: Props) => {
  ...
  const {loading, error, data} = useQuery<GetOrderInfo, GetOrderInfoVariables>(
    BITQUERY_ORDER_INFO,
    {
      variables: {
