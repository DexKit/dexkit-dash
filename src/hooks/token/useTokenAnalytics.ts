...
} from 'services/graphql/bitquery/token/__generated__/GetTokenAnalytics';
...
export const useTokenAnalytics = (
  ...
  useEffect(() => {
    if (dataFn && dataFn?.ethereum?.dexTrades) {
      setData(undefined);
      const sellSide = dataFn?.ethereum?.dexTrades.find(
        (t) => t.side?.toUpperCase() === 'SELL',
      );
      const buySide = dataFn?.ethereum?.dexTrades.find(
      ...
      if (sellSide && buySide) {
        setData({
          ...
          gasValueSpentUSD:
            (sellSide.gasValueUSD || 0) + (buySide.gasValueUSD || 0),
          averageBuyPrice:
            (buySide.amountUSD || 0) /
            (buySide.baseAmount && buySide.baseAmount !== 0
              ? buySide.baseAmount
              : 1),
          averageSellPrice:
            (sellSide.amountUSD || 0) /
            (sellSide.baseAmount && sellSide.baseAmount !== 0
              ? sellSide.baseAmount
              : 1),
