...
} from 'services/graphql/bitquery/affiliate';
import {
  GetAffiliateTrades,
  GetAffiliateTradesVariables,
} from 'services/graphql/bitquery/affiliate/__generated__/GetAffiliateTrades';
import {
  GetTotalAffiliateTradesVariables,
  GetTotalAffiliateTrades,
} from 'services/graphql/bitquery/affiliate/__generated__/GetTotalAffiliateTrades';
...
export const useAffiliateTrades = (affiliateAccount: string, currentChainId: ChainId) => {
  ...
  const totalData = useQuery<
    GetTotalAffiliateTrades,
    GetTotalAffiliateTradesVariables
  >(BITQUERY_TOTAL_AFFILIATE_TRADES, {variables, skip});

  const total = totalData.data?.ethereum?.transfers?.length
    ? totalData.data?.ethereum?.transfers[0].count
    : 0;
  const valueTotalUSD = totalData.data?.ethereum?.transfers?.length
    ? totalData.data?.ethereum?.transfers[0].amountUSD
    : 0;
