...
export const useAMMPairExplorer = ({exchange, address, networkName}: Props) => {
  ...
  useEffect(() => {
    if (dataFn && dataFn?.ethereum?.dexTrades && dataFn?.ethereum?.dexTrades.length > 1) {
      ...
      const basePerDolar = (d24Current.baseAmountInUsd||0) / (d24Current.baseAmount||1);
      const quotePerDolar = (d24Current.quoteAmountInUsd||0) / (d24Current.quoteAmount||1);

      const basePooled = pooled.find((e: any) => e.currency.symbol === d24Current.baseCurrency?.symbol)?.value || 0;
      const quotePooled = pooled.find((e: any) => e.currency.symbol === d24Current.quoteCurrency?.symbol)?.value || 0;

      d24Current['price'] = Number(d24Current.close_price);
      d24Current['priceUsd'] = ( Number(d24Current.close_price) * quotePerDolar);
      d24Current['priceChange'] = ((Number(d24Current.close_price)* 100) / Number(d24Yesterday.close_price)) - 100;
      d24Current['liquidity'] = ((basePooled * basePerDolar) + (quotePooled * quotePerDolar));
