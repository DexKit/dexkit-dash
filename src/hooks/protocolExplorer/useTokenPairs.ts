...
export const useTokenPairs = ({baseAddress, exchange}: Props) => {
  ...
  useEffect(() => {
    if (dataFn && dataFn?.ethereum?.dexTrades) {
      let dexTrades: any[] = [...dataFn.ethereum?.dexTrades] || [];

      const newData = dexTrades.map(e => {
        const j = {...e};

        const quotePerDolar = (j.quoteAmountInUsd||0) / (j.quoteAmount||1);
        ...
        return j;
      }).sort((a, b) => (b?.tradeAmountInUsd || 0) - (a?.tradeAmountInUsd || 0));
